...
import com.xinhai.security.shiro.image.CustomImageCaptchaServlet;
...
public class JCaptchaValidateFilter extends AccessControlFilter {
  ...
  /**
   * 是否允许访问 请求方式，登录的时候会有两个请求，一个是GET请求返回登录页面(这个就不需要走验证码过滤器)，一个是POST请求用于用户登录（需要验证码过滤器）。
   * 如果jCaptchaDisabled是false或者请求方式是POST就验证输入的验证码是否正确。 如果jCaptchaDisabled是true或者请求不是POST那么就不需要验证码，直接允许通过。
   *
  ...
  protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) throws Exception {
    ...
    return httpServletRequest.getSession(false) != null && CustomImageCaptchaServlet.validateResponse(httpServletRequest, jCaptchaCodeParamter.toLowerCase());
  }
