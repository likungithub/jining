...
var taskFiles = function () {
    ...
    var savefile = function () {
        ...
        if(!sessionStorage.getItem("fj_fjid")){
            configMap.jlid="jlid";
        }
        var url = configMap.path + configMap.dataUrl+"/"+configMap.rwid+"/"+configMap.bzid+"/"+configMap.jlid;
        ...
        var inputform = {
            ...
            success: function (data) {
                ...
                if (data.success) {
                    ...
                    sessionStorage.setItem("fj_fjid",data.jlid);
                    /*if("jlid"==configMap.jlid) {
                        configMap.jlid = data.jlid;
                        $("#jlid_fj_id", jqueryMap.$contractForm).val(configMap.jlid);
    ...
    var getFileList = function () {
        configMap.jlid = sessionStorage.getItem("fj_fjid");
        var url = configMap.path + configMap.dataUrl+"/"+configMap.rwid+"/"+configMap.bzid+"/"+configMap.jlid;
        var type = "GET";
        $.ajax({
            url: url,
            ...
            type: type,
            success: function (result) {
                localStorage.setItem('fileNum',result.length);
                ...
                jqueryMap.$contractForm.find('li').off('click').off('click', function () {
    ...
    return {
        // 初始化
        init: function (rwid,bzid) {
            configMap.rwid=rwid;
            configMap.bzid=bzid;
            configMap.jlid=$("#jlid_fj_id").val();
            configMap.jlid = sessionStorage.getItem("fj_fjid");
            // configMap.uuid = uuid;
            //获取附件
            getFileList();
