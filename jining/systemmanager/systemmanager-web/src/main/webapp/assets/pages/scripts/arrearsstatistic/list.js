 ...
 * 
 */
var chargeorderliststatistics = function () {
	'use strict';
// 全局属性参数
	var configMap = {
		path: '',
		dataUrl: '/charge/chargelistByArrears',
		idjson:'',
        ...
        $.ajax({
            ...
            success: function (datas) {
                ...
                if(datas.length>0){
                    ...
                    $('#inputPeople',jqueryMap.$content).append(op);
					$('#customerZhuguan',jqueryMap.$content).append(op);
					$('#customerMaster',jqueryMap.$content).append(op);
                }
            }
        });
    };
    ...
    var getSFXM = function (){
        $.ajax({
            ...
            success: function (datas) {
                if (datas.length > 0) {
                    var datahtml='<option value="999" selected="selected">全部</option>';
                    for(var i=0;i<datas.length;i++){
		...
		configMap.chargeauditGrid = jqueryMap.$manualdata.DataTable({
            ...
            "ajax": {
                ...
                "data": function (data) {
                	...
                	var fwzt=$('#serviceStat',jqueryMap.$content).val();
                	var sfqd=$('#chargeAccount',jqueryMap.$content).val();
                	var sfr=$('#inputPeople',jqueryMap.$content).val();
                	var khzg=$('#customerZhuguan',jqueryMap.$content).val();
    ...
    var exportExcel = function () {
        ...
        var fwzt=$('#serviceStat',jqueryMap.$content).val();
        var sfr=$('#inputPeople',jqueryMap.$content).val();
        window.location.href = configMap.path + "charge/downDataExcelByArreal?searchtxt=" + searchtxt + "&more=" + more + "&auditStatic=" + auditStatic
			+ "&chargeStatic=" + chargeStatic + "&chargeType=" + chargeType + "&chargeModel=" + chargeModel + "&chargeMode=" + chargeMode + "&khbm=" + khbm+"&fwzt=" + fwzt + "&sfqd=" + sfqd + "&sfr=" +sfr;
    };
	return {
		init: function (uuid,type,khbm) {
            setJqueryMap(uuid);
            var tabid=$('#chargeorder_id_div_'+uuid).parents('.tab-pane').attr('id').slice(17);
            tabMenu(tabid,chargeorderliststatistics);
            getAllChargeAccount();
            getZY();
            getSFXM();																									//获取当前代理机构的收费项目
            configMap.khbm = khbm;
            if (type === 'CFTX') { //催费提醒
            	$('[name="auditStatic"]',jqueryMap.$content).val("999"); //审核状态（同意）
			...
			jqueryMap.$content.find('#MoreSearch_btn').off('click').on('click',function (){
				...
				if(showflag=="0"){
                    $("#MoreSearch_btn").next().removeClass("rotate1")
                    ...
                    $("#MoreSearch_btn").next().addClass("rotate1")
