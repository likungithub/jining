var lysp1List = function () {
    var configMap = {
        dataUrl: "/lysp/lysp1Seach",
        saveZtUrl: "/lysp/saveZt1",
        lysp1Grid: null,
        path: ""
    };
    // 全局Dom
    ...
    var setJqueryMap = function (uuid) {
        ...
        jqueryMap.$ManageDataTable = $('#lysp1_list', jqueryMap.$container);
    };
    ...
    var initGrid = function () {
        configMap.lysp1Grid = jqueryMap.$ManageDataTable.DataTable({
    ...
    var saveZt = function (lx) {
        ...
        jqueryMap.$container.find('[name="check"]:checked').each(function () {
            var el = $(this);
            var rowIndex = configMap.lysp1Grid.cell(el.parent()).index().row;
            var id = configMap.lysp1Grid.row(rowIndex).data().id;
            ids.push(id);
        });
        if (ids.length == 0) {
            Messenger().post({
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.success) {
                        ...
                        configMap.lysp1Grid.ajax.reload();
                    } else {
                        Messenger().post({
                            message: "保存失败!",
                            type: 'warning'
    ...
    return {
        inint: function (uuid) {
            setJqueryMap(uuid);
            initGrid();
            jqueryMap.$container.find("#lysp1_tg").on("click", function () {//通过
                saveZt("001");
            });
            jqueryMap.$container.find("#lysp1_th").on("click", function () {//退回
                saveZt("002");
            });
            jqueryMap.$container.find("#lysp1_cx").on("click", function () {//查询
                configMap.lysp1Grid.ajax.reload();
            });
            jqueryMap.$container.find("#lysp1_cz").on("click", function () {//重置
