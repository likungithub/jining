...
<%--										<input type="hidden" class="inputCommon appsysinfo-m" id="lqsj" value="<%=txtDate%>" style="outline:none;border: 0;text-indent:0px !important;width:214px !important;font-size:12px !important;" />--%>
								...
								<div style="clear: both;">
									...
									<div class="input-group  search-label-small pull-left"
										...
										<select class="form-control" id="cplb<%=uuid%>" style="width: 214px">
											...
											<option value="001" selected>未领取</option>
										</select>
									</div>
									<%--<div class="input-group  search-label-small pull-left"--%>
										 <%--style="left: -3px;margin-left: 3px;position:relative;;width:278px!important;border-radius: 4px!important;overflow: hidden;">--%>
								...
								<div style="clear: both;">
									...
									<%--<button id="btn_dyjjdjb" class="btn btn-default fffBg btnBorderColor colorBlue-10a0f7 mr pull-left borderRadius4"><i class="fa fa-plus iconMr"></i>打印交接登记表</button>--%>
									<button id="btn_dyypbm<%=uuid%>" class="btn btn-default fffBg btnBorderColor colorBlue-10a0f7 mr pull-left borderRadius4" style="display: none"><i class="fa fa-plus iconMr"></i>打印样品编号</button>
								</div>
								<!--按钮  end-->
							</div>
						</div>
					...
					<table class="table table-striped table-bordered table-hover" id="list_data<%=uuid%>" name="ypjs-table" style="width:100%;margin-top: 15px!important">
						<thead>
							<tr>
								...
								<th width="120px">产品执行标准</th>
								<th width="80px">抽样日期</th>
								<th width="12%">领取人</th>
								<th width="12%">领取时间</th>
								<th width="12%">备注</th>
...
<script src="<%= request.getContextPath()%>/assets/pages/scripts/marketManage/ypgl/yplqfc/ypLqListFc.js" type="text/javascript"></script>
<script type="text/javascript">
	$(function() {
		ypLqList.setPath("<%= request.getContextPath()%>");
		ypLqList.init("<%=uuid %>");
		//全选
