...
var fileAdd = function () {
    ...
    var checkContract = function () {
        var url = configMap.path + configMap.checkUrl;
        url = url + "/" + configMap.htbm;
        $.ajax({
            url: url,
            dataType: 'JSON',
            type: 'GET',
    ...
    var getFileList = function () {
        var uuid = configMap.uuid;
        var url = configMap.path + configMap.dataUrl;
        var type = "GET";
        if (configMap.htbm != '') {
            url = configMap.path + configMap.dataUrl + "/" + configMap.htbm;
        } else {
            url = configMap.path + configMap.dataUrl + "/" + uuid;
        }
        $.ajax({
            url: url,
            dataType: 'JSON',
            ...
            success: function (result) {
                ...
                jqueryMap.$contractForm.find('li').off('click').on('click', function () {
                    ...
                    rightcontent += '<div class="col-md-12"><span><a onclick="fileAdd.getURL(' + '\'' + result[indexsize].fjcclj + '\',' + '\'' + result[indexsize].fjmc + '\',' + '\'' + result[indexsize].id + '\'' + ')" name="downloadfile">下载</a></span></div>';
                    rightcontent += '</div></div></div>';
                    $("#contractFileAdd").append(rightcontent);
                });
    ...
    return {
        // 初始化
        init: function (id, uuid, htbm, status) {
            configMap.id = id;
            configMap.htbm = htbm;
            configMap.status = status;

            //将当前获取到的uuid放如session中
            if (htbm == null || htbm == undefined || htbm == "") {
                if (sessionStorage.getItem("fileuuid") == null || sessionStorage.getItem("fileuuid") == undefined || sessionStorage.getItem("fileuuid") == "") {
                    sessionStorage.setItem("fileuuid", uuid);
                    configMap.uuid = uuid;
                } else {
                    configMap.uuid = sessionStorage.getItem("fileuuid");
                }
            }
            //获取附件
            getFileList();
