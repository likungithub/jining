 ...
 *
 */
var ypjsjbxx = function () {
    'use strict';
// 全局属性参数
    var configMap = {
        path: '',
        editUrl: '/ypgl/getTypjsAlYpxq',
    ...
    var setJqueryMap = function (uuid) {
        ...
        jqueryMap.$content = $('#ypjs_jbxx' + uuid);
        ...
        configMap.ypjsGrid = jqueryMap.$manualdata.DataTable({
            ...
            "ajax": {
                ...
                "data": function (data) {
                    ...
                    data.jszt = $('[name="ypzt_search"]',jqueryMap.$content).val();
                    data.cpdlmc = $('[name="cpdlmc"]',jqueryMap.$content).val();
                    data.jcxm = $('[name="jcxm"]',jqueryMap.$content).val();
                    data.zxbz = $('[name="zxbz"]',jqueryMap.$content).val();
                    data.BgstartTime = $('[name="BgstartTime"]',jqueryMap.$content).val();
                    data.BgendTime = $('[name="BgendTime"]',jqueryMap.$content).val();
 ...
 var openModal1 = function (title, url, type) {
            ...
            dialogButtons.success = {
                ...
                callback: function () {
                    ...
                    jqueryMap.$content.find('[name=ck]:checked').each(function () {
                        ...
                        var el = $(this);
                        var rowIndex = configMap.ypjsGrid.cell(el.parent().parent()).index().row;
                        var id = configMap.ypjsGrid.row(rowIndex).data().ypbm;
                        strArr.push(id);
                    });
    ...
    return {
        init: function (id,uuid,wtid) {
            ...
            var tabid = $('#ypjs_jbxx' + uuid).parents('.tab-pane').attr('id').slice(17);
            tabMenu(tabid, ypjsjbxx);
