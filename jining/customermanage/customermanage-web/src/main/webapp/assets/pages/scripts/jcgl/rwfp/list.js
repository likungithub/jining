        ...
        if (zt == "002") {//任务分配
            openModal("检测任务分配", configMap.path + configMap.edit_Url, "fp", function () {
                ...
                jqueryMap.$container.find('[name=checkbox_checkbox]:checked').each(function () {
                    ...
                    // data.jcxmid = data.jcxmid + ',' + configMap.listGrid.row(rowIndex).data().jcxmid;
                    // data.ypid = data.ypid + ',' + configMap.listGrid.row(rowIndex).data().ypid;
                    // data.id = data.id + ',' + configMap.listGrid.row(rowIndex).data().id;
