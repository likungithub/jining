...
list = function () {
        ...
        configMap.listGrid = $('#' + configMap.uuid + 'ManagerList_m', jqueryMap.$container).DataTable({
            ...
            "ajax": {
                ...
                "data": function (data) {
                    ...
                    data.bgzt= $("#bgzt", jqueryMap.$container).val();
                }
            },
            "columns": [
                {
    ...
    var ckZfwt = function (){
        ...
        generateTab(this, configMap.path + "/zfwt/ckzfwt" + '?id=' + id, "查看抽样信息", "ckzfwt_info", 'fa fa-file-text-o iconMr');
    }
    ...
    function th() {
        $("#shtjthyy").off("click").on("click",function () {
            savezt('004');
        });
    }
    //报告打印、流转单打印
    ...
    return {
        init: function (uuid) {
            ...
            setJqueryMap();
            initlistGrid();
            $('#bgsh_searchTerm', jqueryMap.$container).on('click', function () {
                configMap.listGrid.ajax.reload();
            });
