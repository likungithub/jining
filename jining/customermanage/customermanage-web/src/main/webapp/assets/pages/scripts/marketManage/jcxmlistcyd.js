...
// 全局属性参数
    ...
    var initJcxmGrid = function () {
        configMap.jcxmGrid = jqueryMap.$manualdata.DataTable({
            ...
            "ajax": {
                ...
                "dataSrc": "aaData",
                "data": function (data) {
                    data.ypid = configMap.from == 'ypzb' ? configMap.ypid : '';
                    ...
                    data.jclbdm = $("#z_jcxmlb"+configMap.uuid).val();
                }
            },
            "columns": [
                {
                    ...
                    "render": function (data, type, row) {
                        console.log(configMap.selectlist ,data)
                        if(configMap.selectlist.indexOf(data.toString())>=0){
                            return '<input type="checkbox" checked name="checkbox_checkbox" value="'+data+'" id="jcxm_' + data + '"/>';
                        }
    ...
    var saveYpJcxm = function () {
        ...
        var data = {
            jcxmId:jcxmjson,
            ypids:configMap.ypids,
            ypzbid:configMap.ypzbid
        };
        var url = configMap.path + (configMap.from == 'ypzb' ? '/jcxm/saveYpzbJcxm': '/jcxm/saveYpJcxmcyd')
        console.log(configMap.from,url)
        $.ajax({
            url:url,
            type:'POST',
    ...
    var getSelected = function() {
        var url = configMap.path + (configMap.from == 'ypzb' ? '/jcxm/getYpzbJcxm' : '/jcxm/getYpJcxm') + '?ypid='+configMap.ypids+'&ypzbid='+configMap.ypzbid
        $.ajax({
            type:'get',
            url: url, 
            dataType:'json',
            success: function(data){ 
                if(data.success){
                    configMap.selectlist = _.map(data.data,'jcxmid');
                    console.log(configMap.selectlist)
                    $('input[name="checkbox_checkbox"]',jqueryMap.$content).each(function(idx,el){ 
                        var rowIndex = configMap.ypjsGrid.cell($(el).parent()).index().row;
                        var tid = configMap.ypjsGrid.row(rowIndex).data().ID;
                        console.log(tid)
                        if(configMap.selectlist.indexOf(tid) >=0 ){
                            el.checked = true
                        } 
                    });
                }
    ...
    return {
        init: function (uuid,ypids,from,ypzbid) {
            configMap.uuid = uuid;
            configMap.ypids = ypids;
            configMap.ypid = ypids;
            configMap.from = from;
            configMap.ypzbid = ypzbid;
            setJqueryMap(uuid);
            getSelected()
            ...
            $("#reset",jqueryMap.$content).off('click').on('click',function (){
                ...
                $('#z_jcxmlb'+configMap.uuid, jqueryMap.$content).get(0).selectedIndex=0;
                jcxmSearch();
            });
