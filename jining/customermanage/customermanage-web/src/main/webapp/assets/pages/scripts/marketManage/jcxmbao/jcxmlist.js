...
// 全局属性参数
    var configMap = {
        path: '',
        dataUrl: '/jcxm/getJcbJcxmList',
        addUrl:'/marketManage/jcxm_jbxx.jsp',
        datatablesLanguageFile: '/assets/global/plugins/datatables/chinese.json',
        jcxmGrid: null,
        editBtn_html: '<a href="javascript:;" class="btn btn-xs default"  data-toggle="tooltip" data-placement="bottom"  name="contractedit" title="修改检测项信息"><i  class="icon iconfont icon-bianji1 iconFontColor-10a0f7 iconFontSize"></i></a>',
        deleBtn_html: '<a href="javascript:;" class="btn btn-xs default"  data-toggle="tooltip" data-placement="bottom"   name="contractdelete" title="删除检测项信息"><i class="icon iconfont icon-shanchu3 iconFontColor-10a0f7 iconFontSize"></i></a>',
        fwzt: '',
        other: '',
        uuid: '',
        ypid: '',
    ...
    var setJqueryMap = function (uuid) {
        ...
        jqueryMap.$content = $('#jcxm1' + uuid);
        jqueryMap.$manualdata = jqueryMap.$content.find('#jcxmlist_data1');
    };

    var jcxmjson = [];

    var initJcxmGrid = function () {
        console.log(jqueryMap.$manualdata)
        configMap.jcxmGrid = jqueryMap.$manualdata.DataTable({
            ...
            "ajax": {
                ...
                "data": function (data) {
                    ...
                    data.cpdl = $('input[name="cpdl"]',jqueryMap.$content).val();
    ...
    var editJcxm = function (){
        ...
        generateTab(this, configMap.path + configMap.addUrl + '?id=' + id, "修改检测项信息", "jcxm_info", 'fa fa-file-text-o iconMr');
    };
    ...
    var saveYpJcxm = function () {
        ...
        $.ajax({
            url:configMap.path + '/jcb/addjcx?jcxmid',
            type:'POST',
    ...
    //刷新事件
    $('#jcxmlist_data1').on('page.dt',function() {
        var inputjson = $('[type="checkbox"]:checked', jqueryMap.$manualdata).not(jqueryMap.$content.find('[name="selectjcxmlist"]'));
    ...
    return {
        init: function (uuid,jcbid) {
            configMap.uuid = uuid;
            configMap.jcbid = jcbid;
            setJqueryMap(uuid);
            var tabid = $('#jcxm1' + uuid).parents('.tab-pane').attr('id').slice(17);
            ...
            $("#reset",jqueryMap.$content).off('click').on('click',function (){
                ...
                $('input[name="pdyj"]',jqueryMap.$content).val('');
                $('input[name="cpdl"]',jqueryMap.$content).val('');
