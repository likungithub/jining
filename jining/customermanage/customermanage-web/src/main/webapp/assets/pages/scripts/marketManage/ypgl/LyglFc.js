var lyglList = function () {
    var configMap = {
        path: '',
        dataUrl: "customermanage/lyglFc/findAll",
    ...
    var spCgsq = function () {
        ...
        jqueryMap.$content.find('[name=che]:checked').each(function () {
            ...
            jcxmids.push(configMap.$lyglGrid.row(rowIndex).data().jcxmid);
            ypids.push(configMap.$lyglGrid.row(rowIndex).data().ypid);//zbwc表的 ypid
            ids.push(configMap.$lyglGrid.row(rowIndex).data().id);//zbwc表的id
        });
        data.jcxmid = jcxmids.join(";");
        data.ypid = ypids.join(",");
        data.id = ids.join(",");
        console.log(data.jcxmid);
        console.log(data.ypid);
        console.log(data.id);
        $.ajax({
            url: "customermanage/lyglFc/updatelqzt",
            type: 'POST',
            data: data,//$("#lygl_form", jqueryMap.$content).serialize(),
            async: false,
    ...
    var deletedBtn = function () {
        ...
        $.ajax({
            url: "customermanage/lyglFc/deletedlqzt",
            type: 'POST',
