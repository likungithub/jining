...
// 全局属性参数
        ...
        configMap.jcxmGrid = jqueryMap.$manualdata.DataTable({
            ...
            "ajax": {
                ...
                "data": function (data) {
                    ...
                    data.pdyj = $('input[name="pdyj"]',jqueryMap.$content).val();
                    data.jyyj = $('input[name="jyyj"]',jqueryMap.$content).val();
                }
            },
            "columns": [
                {
                    ...
                    "render": function (data, type, row) {
                        return '<input type="checkbox" name="jcxmids_chk" value="'+data+'" id="jcxm_' + data + '"/>';
                    }
                },
                {
                    class:"text-center",
            ...
            "drawCallback": function () { // 数据加载完成后执行
                ...
                jcxmJson.data.forEach(function(item){
                    $('input[value="'+item.id+'"]',jqueryMap.$content).prop('checked',true)
                })
                $('input[name="jcxmids_chk"]',jqueryMap.$content).off('change').on('change',function(){
                    var el = $(this);
                    console.log(el)
                    var rowIndex = configMap.jcxmGrid.cell(el.parent()).index().row;
                    var id = configMap.jcxmGrid.row(rowIndex).data().id;
                    var namestr = configMap.jcxmGrid.row(rowIndex).data().zwmc_bm;
                    if (el.prop("checked")) {
                        //选中
                    ...
                    $('#add_jcxm',jqueryMap.$content).text(_.map(jcxmJson.data,'name').join(','))
                });
            }
        });
    };
    function addJson(id, name){
        var index  = _.findIndex(jcxmJson.data,function(item){
            return item.id == id
        })
        if(index<0){
            jcxmJson.data.push({
    ...
    function removeJson(id,name){
        ...
        if(index>=0){
            _.remove(jcxmJson.data,function(item){
                return item.id == id
            })
        }
    }
        ...
        $.get(configMap.path + '/jcxm/getCpmc/' + type , function (res) {
            if (res.success) {
                ...
                if (type == '002') {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        if (data[i].YL && data[i].YL != null)
                        $('<option value="' + delnull(data[i].YL) + '">' + delnull(data[i].YL) + '</option>').appendTo($('#yl' + configMap.uuid, jqueryMap.$content));
                    }
                }
                if (type == '003') {
                    $('#cyl' + configMap.uuid, jqueryMap.$content).empty();
                    for (var i = 0; i < data.length; i++) {
                        if (data[i].CYL && data[i].CYL != null)
                            $('<option value="' + delnull(data[i].CYL) + '">' + delnull(data[i].CYL) + '</option>').appendTo($('#cyl' + configMap.uuid, jqueryMap.$content));
                    }
                }
                if (type == '004') {
    ...
    var selectAll = function (status) {
        ...
        var inputjson = $('[type="checkbox"]', jqueryMap.$manualdata).not(jqueryMap.$content.find('[name="selectjcxmlist"]'));
        console.log(inputjson);
        if (status) {
            $('[type="checkbox"]:checked', jqueryMap.$manualdata).not(jqueryMap.$content.find('[name="selectjcxmlist"]')).each(function () {
                var el = $(this);
                var rowIndex = configMap.jcxmGrid.cell(el.parent()).index().row;
                var id = configMap.jcxmGrid.row(rowIndex).data().id;
                var namestr = configMap.jcxmGrid.row(rowIndex).data().zwmc_bm;
            ...
            $('[type="checkbox"]', jqueryMap.$manualdata).not(jqueryMap.$content.find('[name="selectjcxmlist"]')).each(function () {
                var el = $(this);
                console.log(el);
                var rowIndex = configMap.jcxmGrid.cell(el.parent()).index().row;
                var id = configMap.jcxmGrid.row(rowIndex).data().id;
    ...
    var saveYpJcxm = function () {
        stopContinueClick("#saveYpJcxm", 300);
      /*  var ids=[];//定义一个数组
        ...
        var jcxmNameArr = [];
         var nameTemp = null;
         $(jcxmJson.data).each(function (index,element) {
             // console.log(element);
             temp = {jcxmId: element.id};
             nameTemp = {jcxmName:element.name}
             jcxmArr.push(temp);
             jcxmNameArr.push(nameTemp);
         });
        var data = {
            jcxmId:jcxmArr,
            jcxmName:jcxmNameArr,
            wtid:configMap.wtid
        };
        $.ajax({
            // url:configMap.path + '/jcxm/saveYddypJcxm',
            //20190910添加重名不保存
            url:configMap.path + '/jcxm/saveYddypJcxmNew',
            type:'POST',
