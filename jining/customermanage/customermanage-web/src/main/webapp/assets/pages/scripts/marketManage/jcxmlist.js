...
// 全局属性参数
    var configMap = {
        ...
        dataUrlNew:'/jcxm/getJcxmListNew',
        addUrl:'/marketManage/jcxm_jbxx.jsp',
        datatablesLanguageFile: '/assets/global/plugins/datatables/chinese.json',
        jcxmGrid: null,
        editBtn_html: '<a href="javascript:;" class="btn btn-xs default"  data-toggle="tooltip" data-placement="bottom"  name="contractedit" title="修改检测项目"><i  class="icon iconfont icon-bianji1 iconFontColor-10a0f7 iconFontSize"></i></a>',
    ...
    var initJcxmGrid = function () {
        configMap.jcxmGrid = jqueryMap.$manualdata.DataTable({
            ...
            "scrollX":true,//水平滚动
            "ajax": {
                "url": configMap.path + configMap.dataUrlNew,
                "dataSrc": "aaData",
                "data": function (data) {
                    ...
                    data.dlmc = $('input[name="dlmc"]',jqueryMap.$content).val();
                    data.cylmc = $('input[name="cylmc"]',jqueryMap.$content).val();
                    data.jcff = $('input[name="jcff"]',jqueryMap.$content).val();
    ...
    //增加 加测项
    var jcxmAdd = function () {
        stopContinueClick("#addCustomerManage", 300);
        generateTab(this, configMap.path + "/marketManage/jcxm_addjbxx.jsp", "增加检测项", "jcxm_info", 'fa fa-file-text-o iconMr');
    };
    ...
    var saveYpJcxm = function () {
      /*  var ids=[];//定义一个数组
        ...
        $.ajax({
            url:configMap.path + '/jcxm/saveYpJcxm',
            type:'POST',
    ...
    return {
        init: function (uuid,wtid) {
            ...
            initJcxmGrid();
            $("#jcxmAdd",jqueryMap.$content).off('click').on('click',function (){
                jcxmAdd();
            });
