...
// 全局属性参数
	...
	var setJqueryMap = function (uuid) {
		...
		jqueryMap.$content = $('#contractchangeview_id_div_'+uuid);
		jqueryMap.$manualdata=jqueryMap.$content.find('table#contractchangeauditview_data');
        jqueryMap.$htTree = $('#htbgview_tree', jqueryMap.$content);
	};
    ...
    var initOrganization = function () {
        ...
        //输入框输入定时自动搜索
        var to = false;
        $('#search_Htbgvieway').keyup(function () {
            if (to) {
                clearTimeout(to);
            }
            to = setTimeout(function () {
                jstree.jstree(true).search($('#search_Htbgvieway').val());
            }, 250);
        });
	...
	return {
		init: function (uuid,bz) {
			setJqueryMap(uuid);
			configMap.bz = bz;
			var tabid=$('#contractchangeview_id_div_' + uuid).parents('.tab-pane').attr('id').slice(17);
            tabMenu(tabid);
		...
		setPath: function (path) {
			configMap.path = '/statisticalanalysis';
		}
	};
}();
//@ sourceURL=log.js
