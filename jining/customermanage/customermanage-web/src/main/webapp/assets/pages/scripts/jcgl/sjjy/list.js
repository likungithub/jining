    ...
    var initlistGrid = function() {
        configMap.listGrid = $('#'+configMap.uuid+'ManagerList_m', jqueryMap.$container).DataTable({
            ...
            "drawCallback": function() { // 数据加载完成后执行
                ...
                var backContainer = $('[data-type="back"]', jqueryMap.$container);
...
//提交修改状态  通过
    function savezt(zt){
        ...
        jqueryMap.$container.find('[name=checkbox_checkbox]:checked').each(function () {
            ...
            var rowIndex = configMap.listGrid.cell(el.parent()).index().row;
            var id = configMap.listGrid.row(rowIndex).data().ID;
            var shzt = configMap.listGrid.row(rowIndex).data().SJJYZT;
            if(shzt!='001'){ //通过
                flag = true;//直接退出
                return;
            }
    ...
    return {
        init: function(uuid,jclbdm) {
            ...
            //yuanao,按编码查询按钮的，重加载事件  0713
            $('#searchTerm-b', jqueryMap.$container).on('click', function () {
               // alert(123);
                configMap.listGrid.ajax.reload();
            });
            ...
            //yunaao，重置   0713
            $("#cz",jqueryMap.$container).off('click').on('click',function (){
            ...
            //通过
            $($('#'+uuid+'btn_sjjy_tg')).off('click').on('click',function(){//通过
                //通过 002
                savezt('002');
            });
