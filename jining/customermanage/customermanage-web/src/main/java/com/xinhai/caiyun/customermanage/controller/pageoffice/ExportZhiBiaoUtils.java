...
public class ExportZhiBiaoUtils {
    // 显示的导出表的标题
    ...
    // 构造方法，传入要导出的数据
    public ExportZhiBiaoUtils(String title, String[] rowName,
                            List<Object[]> dataList,String ypbm) {
    ...
    public void export(OutputStream out) throws Exception {
        try {
            ...
            HSSFCell cellTiltle = rowm.createCell(3);
            HSSFRow rowm1 = sheet.createRow(1);
            HSSFRow rowm2 = sheet.createRow(2);
            HSSFRow rowm3 = sheet.createRow(3);
            sheet.addMergedRegion(new CellRangeAddress(0,3,0,0));
            HSSFCell cellTable = rowm.createCell(0);
            cellTable.setCellValue("受检单位名称");
            rowm.createCell(1).setCellValue("样品名称");
            rowm.createCell(2).setCellValue("样品编号");
            rowm2.createCell(2).setCellValue(ypbm);
             ...
             * (rowName.length-1)));
             */// 合并单元格
            sheet.addMergedRegion(new CellRangeAddress(0, 0, 3,
                    dataList.get(0).length-1 ));// 列行
            cellTiltle.setCellStyle(style);
            cellTiltle.setCellValue("检测结果   mg/kg");
            ...
            for (int n = 0; n < columnNum; n++) {
                HSSFCell cellRowName = rowm1.createCell(n+3); // 创建列头对应个数的单元格
                cellRowName.setCellType(HSSFCell.CELL_TYPE_STRING); // 设置列头单元格的数据类型
                HSSFRichTextString text = new HSSFRichTextString(rowName[n]);
                cellRowName.setCellValue(text); // 设置列头单元格的值
                cellRowName.setCellStyle(columnTopStyle); // 设置列头单元格样式
                sheet.addMergedRegion(new CellRangeAddress(1,3,n+3,n+3));
            }

            // 将查询出的数据设置到sheet对应的单元格中
            for (int i = 0; i < dataList.size(); i++) {
                Object[] obj = dataList.get(i);// 遍历每个对象
                HSSFRow row = sheet.createRow(i + 4);// 创建所需的行数（从第三行开始写数据）
