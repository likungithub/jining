...
@Controller
@RequestMapping("ypglZbxx")
public class YpglZbxxCobtroller {
    ...
    //获取制备拆分表
    @ResponseBody
    @RequestMapping("/getKsListzb")
    public DatatablesViewPage getKsListzb(HttpServletRequest request){
        List list = new ArrayList();
        Map map = new HashMap();
        String ypids = request.getParameter("ypids");
        String ypid="";
        if(ypids!=null && ypids.indexOf(",")!=-1)
        {
            //表示 ypids 是多个 样品  多个样品批量制备 只能取 第一个 因为 批量 表示 都用同一种科室和数量
            ypid = ypids.substring(0,ypids.indexOf(","));
        }else//只有一个 样品id
        {
            ypid=ypids;
        }
        map.put("ypid",ypid);
        list=this.tlZypglMapper.getZbkszb(map);
        Integer count = this.tlZypglMapper.getZbksNumzb(map);
    ...
    public DatatablesViewPage getKsList(HttpServletRequest request){
        List list = new ArrayList();
        Map map = new HashMap();
        String ypids = request.getParameter("ypids");
        String ypid="";
        ...
        map.put("ypid",ypid);
        list=this.tlZypglMapper.getZbks(map);
        Integer count = this.tlZypglMapper.getZbksNum(map);
        DatatablesViewPage datatablesViewPage = new DatatablesViewPage();
        datatablesViewPage.setAaData(list);
        datatablesViewPage.setiTotalRecords(count);
        datatablesViewPage.setiTotalDisplayRecords(count);
    ...
    public com.alibaba.fastjson.JSONObject addYpzbList(HttpServletRequest request){
        ...
        String ypids = request.getParameter("ypids");
        String wtids = request.getParameter("wtids");
        String ypmcs = request.getParameter("ypmcs");

        //制备总数量所有的通用
        String zbzl = request.getParameter("lrzsl");
        String czryDm =  CurrentLoginUser.getUser().getZydm();

        String zbks_dms[] = request.getParameterValues("zbks_dm");//制备科室代码
        String zbsls[] = request.getParameterValues("zbsl");//制备 数量 带着 总量 要区分

        String ypidArr[] = ypids.split(",");
        String wtidArr[] = wtids.split(",");
        String ypmcArArr[] = ypmcs.split(",");

        List zbYpList = new ArrayList();
        //这个循环是 一次 是一个 抽样  里面还要嵌套  样品下的 科室 循环
        for(int i=0;ypidArr!=null &&i<ypidArr.length;i++)
        {
            String ypid = ypidArr[i];
            String wtid = wtidArr[i];
            String ypmc = ypmcArArr[i];

            for(int j=0;zbks_dms!=null && j<zbks_dms.length;j++)
            {
                String zbsl = zbsls[j]; //制备数量
                String lx = zbks_dms[j];//制备科室代码  之所以用类型 是因为以前 项目设计写死 要改 前后几十个地方 会有问题
                //999说明是总数量
                if(lx!=null && lx.equals("999"))
                {
                    continue;
                }
                //制备数量为0的 不做制备数据
                if(zbsl==null || zbsl.trim().length()==0 || zbsl.trim().equals("0"))
                {
                    continue;
                }
                Map map = new HashMap();

                map.put("zbypbm",wtid+"-检样"+(j+1));
                map.put("ypbm",wtid);
                map.put("ypmc",ypmc);
                map.put("zbfs","1");
                map.put("zbzl",zbsl);
                map.put("dw","g");
                map.put("zbrydm",czryDm);
                map.put("ccwz","");
                map.put("ypid",ypid);
                map.put("lx",lx);
                map.put("zbzs",zbzl);//总数量 也就是 lx 和 制备科室 DM是999的
                zbYpList.add(map);
            }
        }
        String zydm = CurrentLoginUser.getUser().getZydm();
        this.tlZypglMapper.saveYpzbAks(zbYpList,zydm);
    ...
    //肥城样品制备 获取列表
    @RequestMapping(value = "/getzZyplist", method = RequestMethod.POST)
    @ResponseBody
    public DatatablesViewPage getzZyplist(HttpServletRequest request) {
        ...
        long totalCount = tlZypglMapper.getZbYpListNum(cxtj);
        List<Map> list = tlZypglMapper.getZbYpList(cxtj);
     ...
     * @return
     */
    @RequestMapping(value = "/addYpzbFsList")
    @ResponseBody
    public com.alibaba.fastjson.JSONObject addYpzbFsList(HttpServletRequest request){

        String zbfsdms = request.getParameter("zbfsdms");
        String zbwcIds = request.getParameter("zbwcIds");

        String zbwcIdArr[] = zbwcIds.split(",");
        String czryDm =  CurrentLoginUser.getUser().getZydm();


        List zbFsList = new ArrayList();
        //这个循环只一次，制备方式用 一个字段记录，【,】分隔
        for(int i=0;zbwcIdArr!=null &&i<zbwcIdArr.length;i++)
        {
                String zbwcid = zbwcIdArr[i]; //制备完成表的ID
                Map map = new HashMap();
                map.put("ZBWCID",zbwcid);
                zbFsList.add(map);
        }

        this.tlZypglMapper.saveYpzbFs(zbFsList,czryDm,zbfsdms);
    ...
    /**
     * 食品检测——样品制备——肥城 用的 特殊制备  获取已经设置的制备方式
     * @param request
     * @return
     */
    @RequestMapping(value = "/getYpZbfs")
    @ResponseBody
    public String getYpZbfs(HttpServletRequest request){

        String zbwcIds = request.getParameter("zbwcIds");
        String zbwcid="";
        if(zbwcIds!=null && zbwcIds.indexOf(",")!=-1)
        {
            zbwcid = zbwcIds.substring(0,zbwcIds.indexOf(","));
        }else if(zbwcIds!=null)
        {
            zbwcid=zbwcIds;
        }


        String zbfs_dms =  this.tlZypglMapper.getYpZbfs(zbwcid);
        return  zbfs_dms;
    }
...
//
//        }
//        String jssj = obj.getString("lqsj");
...
//        JSONArray jsonObject = JSONArray.fromObject(sb);
