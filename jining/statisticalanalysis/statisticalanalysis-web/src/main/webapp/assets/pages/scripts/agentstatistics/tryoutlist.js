var tryoutlist = function () {
	...
	var configMap = {
		dljgbm:'',
		path: '',//
		dataUrl: '/agentstatistics/tryout',//Controller类中的 @RequestMapping路径/方法名上方的@RequestMapping(value = "????", method = RequestMethod.GET)
        ...
        remindBtn_html: '<button style="background: none;outline: none" class="btn btn-xs default"  data-type="remindarrears" data-toggle="tooltip" title="我要催费"><i class="icon iconfont icon-cuifei  iconFontColor-10a0f7 iconFontSize" style="padding-right:3px;position:relative;top:2px;"></i></button>'
	};
	...
	var setJqueryMap = function (uuid) {
		jqueryMap.$container = $('#tryoutstatistics_id_div_'+uuid);
		jqueryMap.$datatable=jqueryMap.$container.find('table#tryoutstatistics_data');
		...
		configMap.agentGrid = jqueryMap.$datatable.DataTable({
            ...
            "columns": [	
				...
				},
				{
			    	...
			    	"data": "shyj",
			    	"render": function (data, type, row) {
                        return "<a name='khlistt' style='text-decoration:underline'>"+data+"</a>";
                    }
				},
				{	
					className:'text-center',
					"data": "yhid",
					"render": function (data, type, row) {
						return configMap.remindBtn_html + configMap.viewBtn_html;
					}
				}
 			],
 			"language": {
			...
			"drawCallback": function () { // 数据加载完成后执行				
				...
				var khview = jqueryMap.$container.find('[name="khlistt"]');
	...
	var getTryOutNumber = function (){
		$.ajax({
            url: configMap.path + configMap.dataUrl + "number",
            dataType: 'JSON',
            type: 'GET',
            success: function (datas) {
            	$(".tryoutnumber",jqueryMap.$container).html(datas.size);
            },
            error: function () {
            }
        });
	};
    $(".SYclickMore").on("click",function () {
        if($(this).attr("data")==0){
        ...
        $(".SYshowMore").toggle(500);
    })
	return {
		init: function (uuid) {
			setJqueryMap(uuid);
			var tabid=$('#tryoutstatistics_id_div_' + uuid).parents('.tab-pane').attr('id').slice(17);
            tabMenu(tabid);
            initAgentGridByPage();
            getTryOutNumber();
